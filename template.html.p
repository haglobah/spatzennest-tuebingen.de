...
<html lang="de-DE" class="sm:scroll-smooth">
  <head>
	...
	◊(->html `(header ([class "bg-[#44567e] text-[#eeeeff] sticky top-0 z-10"])
				...
				(section ([id "mobile-menu"] [class "fixed z-10 mt-[10vh] h-full pt-6 bg-[#eeeeff] font-roboto text-zinc-700 transition-all duration-200 top-0 w-32 flex-col hidden"])
				  (nav ([class "flex flex-col pb-8 items-center gap-4 justify-evenly"] [aria-label "mobile"])
