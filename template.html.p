  ...
  <head>
	...
	◊(->html `(header ([class "bg-[#44567e] text-[#eeeeff] sticky top-0 z-10"])
				...
				(section ([id "mobile-menu"] [class "fixed mt-[10vh] h-full hidden"])
					...
					(div ([class "fixed z-10 mt-[10vh] top-0 h-full pt-6 bg-[#eeeeff] font-roboto text-zinc-700 transition-all duration-200 w-48 flex flex-col"])
				  	(nav ([class "flex flex-col gap-4 justify-evenly"] [aria-label "mobile"])
							(a ([href "./index.html"] [class "pl-6 text-xl hover:opacity-90"]) "Home")
							(a ([href "./freizeit.html"] [class "pl-6 text-xl hover:opacity-90"]) "Freizeit")
							(a ([href "./mitarbeitende.html"] [class "pl-6 text-xl hover:opacity-90"]) "Mitarbeitende")
							(a ([href "./foerderverein.html"] [class "pl-6 text-xl hover:opacity-90"]) "Förderverein"))))))
